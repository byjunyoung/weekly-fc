...
export function randomAvatar(seed: number): AvatarSpec {
  ...
  // 2026-09-22 이전엔 face·hair·skin·eyes 를 뽑은 바로 다음 호출이 hue였다 — 그 사이에
  // 새 픽을 끼워 넣으면 난수 스트림이 밀려 **한 번도 저장한 적 없는 기존 선수 전원의
  // 기본 유니폼 색이 통째로 바뀐다**(요청 범위 밖의 회귀, opus 리뷰가 실측으로 잡음).
  // hue/kit 을 반드시 예전과 같은 자리(네 필드 추첨 전)에서 뽑아 기존 결과를 보존한다.
