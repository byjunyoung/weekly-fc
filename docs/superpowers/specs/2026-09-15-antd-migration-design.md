...
| 구조 | **페이지마다 React 섬 하나**(`client:load`). Astro 페이지·레이아웃은 남긴다. 규칙 본문처럼 Astro 글이 사이에 끼어 살아 있는 영역이 떨어져 있으면 **영역마다 섬**을 두고, 바뀌지 않는 표 같은 정적 내용은 `client` 지시어 없이 빌드 때만 그린다(JS 0) — 2026-09-15 1단계 계획에서 정함 | 주소·옛 주소 넘김·noindex·빌드 검사를 그대로 둔다. 섬 여럿이 동시에 뜨면 `api.ts`가 데이터 요청을 한 번으로 묶는다 |
...
| 위험 | 근거 | 대응 |
|---|---|---|
| React·antd 설치 파일이 iCloud로 올라감 | 이 레포 `node_modules`는 제외 링크가 아닌 실제 폴더다. `nosync-setup.sh --dry-run` 결과 "이동 + 링크 1곳" | 0단계 첫 작업으로 적용(로컬, 되돌릴 수 있음). `npm install`·`npm ci` 모두 링크를 지우고 실제 폴더를 만든다(0단계 Task 1에서 확인). 설치를 다 끝낸 뒤 옛 `node_modules.nosync`를 지우고 스크립트를 다시 돌린다. 구현 에이전트는 설치하지 않고 컨트롤러가 맡는다 |
