...
> 2026-09-24 전환 뒤 사용자 결정: **백업은 두지 않고 깨우기만.** 데이터가 30명 명단·능력치 정도로
> 작고, 옮긴 날 모습은 구글 시트에 남아 있으며, 능력치는 바뀐 기록이 쌓인다. 반면 7일 멈춤은
> 사이트가 통째로 안 뜨는 일이라 막을 가치가 크다.

이 레포의 `.github/workflows/keepalive.yml` 이 사흘마다 공개 함수 `get_all` 을 한 번 읽는다. 저장하지 않으니
새 저장소·PIN 비밀값이 필요 없다. 서버가 멈췄거나 선수가 0명이면 실패해 GitHub 실패 메일로 알린다.
공개 레포는 60일간 커밋이 없으면 예약 작업이 꺼진다 — 꺼지기 전 메일이 오니 Actions 탭에서 다시 켠다.
