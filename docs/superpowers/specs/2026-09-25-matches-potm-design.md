...
- 팀짜기의 [텍스트 복사]를 없애고 [이미지 저장]으로. **팀이 다 짜였을 때만**(안 정한 사람 없음·팀 둘 이상) 켜진다 — 비활성 버튼에 이유를 툴팁으로.
- `components/teams-image.ts`(1080×1350, 조끼 줄마다 색 띠·아바타·이름, 한 줄 6명, 세로가 모자라면 아바타 축소, 숫자 없음, 용병은 회색 칸)
  · `react/teams/TeamsShareModal.tsx`(티어표 공유 흐름). 짧은 이름 겹침 규칙(`nameMapOf`)은 카톡 텍스트와 같다.
- 매치 카드 머리에도 [이미지 저장](누구나) — "이미지 저장이 매치 목록 뷰에 나와야지". 팀짜기의 버튼은 저장 전 미리 뽑는 용도로 남긴다.
