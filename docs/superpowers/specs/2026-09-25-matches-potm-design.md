...
- `autoBalance(st, players, rnd?)` — 난수가 오면(화면은 `Math.random`) 뱀 드래프트 + 다듬기로 균형을 잡은 뒤 `wander`: ① 팀 번호를 섞고 ② 종합 6 이내로 비슷한 둘을
  **평균 차이가 원래 + 2 안에 머물 때만** 맞바꾸고 ③ 용병은 팀 사이에서 자유롭게 섞는다(머릿수 유지). "+1, 아무나"는 3씩 벌어진 명단에서 한 번도 안 바뀌어 넓혔다. 난수가 없으면 예전처럼 결정적(테스트·대조).

## 5i. 당일만 + 투표 페이지(2026-09-26, "매치 당일만 오픈하고, 공유해서 투표할 수 있게 더 특별하게")

- 창: **매치 당일만**(`potm_open` = today_kst = date, `20260926000000_potm_same_day.sql`). 어제 매치 → '투표는 경기 당일 자정까지입니다', 내일 → '경기 뒤에 투표할 수 있습니다'.
- `/matches/potm/?m=ID`(`react/matches/PotmApp.tsx`) — 금색 히어로(날짜·"POTM 투표"·오늘 자정까지), 후보 카드(얼굴 아바타·이름·득표 막대·내 표 금테·1위 ★),
  [투표 링크 공유](`navigator.share` → 안 되면 클립보드, 글은 `potmShareText`), 전용 미리보기 `og-potm.png`. m 이 없으면 오늘 매치 → 최근 매치.
- 매치 카드: 당일이면 머리에 [🏆 POTM 투표] 버튼만. 카드 안 투표 칩은 뺐다("경로 하나로") — 집계 한 줄("13명 중 2명 투표")만 남는다.
