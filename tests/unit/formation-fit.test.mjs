 ...
 *  (2026-09-22 자리를 아바타+이름으로 바꾸고, 이어서 아바타를 구간별 한계 바로 아래까지
 *   키우며 다시 쟀다. 한계는 scratchpad 의 sweep 으로 구간마다 훑었다 —
 *   예: 900~1119 는 높이 97 까지, 가로 피치는 84 까지 된다. ≥1120 세로는 `.bd-land` 가 명시도로
...
const BANDS = [
  ['≥1120px', 1120, 104, 136, /@media \(min-width: 1120px\)[^@]*?\.bd-slot\s*\{[^}]*?width:\s*(\d+)px/],
  ['900~1119px', 900, 72, 93, /\n\.bd-slot\s*\{[^}]*?width:\s*(\d+)px/],
  ['421~899px', 421, 66, 83, /@media \(max-width: 899px\)[^@]*?\.bd-slot\s*\{[^}]*?width:\s*(\d+)px/],
  ['350~420px', 350, 62, 66, /@media \(max-width: 420px\)[^@]*?\.bd-slot\s*\{[^}]*?width:\s*(\d+)px/],
  ['341~349px', 341, 56, 64, /@media \(max-width: 349px\)[^@]*?\.bd-slot\s*\{[^}]*?width:\s*(\d+)px/],
  ['≤340px', 320, 56, 60, /@media \(max-width: 349px\)[^@]*?\.bd-slot\s*\{[^}]*?width:\s*(\d+)px/],
];
/** 가로 피치 구간 — `.bd-land .bd-slot` 이 미디어 쿼리보다 명시도가 높아 한 벌뿐이다.
 *  가장 좁은 뷰포트는 LineupApp 의 LAND_QUERY(1100px). */
const LAND_BANDS = [
  ['가로 ≥1100px', 1100, 72, 83, /\.bd-land \.bd-slot\s*\{[^}]*?width:\s*(\d+)px/],
];
